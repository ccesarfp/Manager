    ...
    fetch('/persons/roles', {
        ...
        .then(response =>
            response.json()
                ...
                if (res.status === 200) {
                    document.getElementById('role').innerHTML = `<option value="" selected disabled>Select a role</option>`;
                    res.data.forEach(role => {
                        document.getElementById('role').innerHTML += `<option value="${role.name}">${role.name}</option>`;
                    })
                }
            }));
}
    ...
    fetch('/persons/roles', {
        ...
        .then(response =>
            response.json()
                ...
                if (res.status === 200) {
                    document.getElementById('role-update').innerHTML = `<option value="" selected disabled>Select a role</option>`;
                    res.data.forEach(role => {
                        document.getElementById('role-update').innerHTML += `<option value="${role.name}">${role.name}</option>`;
                    })
...
function sendData() {
    ...
    ajax.onreadystatechange = () => {
        if (ajax.readyState === 4 && ajax.status === 200){
            if (type === 'person') {
                reloadScreen(content, '/persons/personsPage', '/persons/', table, personTableTemplate);
            } else if (type === 'employee') {
                reloadScreen(content, '/persons/employeesPage', '/persons/employees', table, employeeTableTemplate)
            } else if (type === 'player') {
                reloadScreen(content, '/persons/playersPage', '/persons/players', table, playerTableTemplate);
            }
...
function putDataConfirmation(id, name) {
    ...
    document.getElementById('text-delete').innerHTML = `Really want to delete: ${name}`;
}
